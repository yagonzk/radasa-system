...
export const pneusGestaoService = {
  ...
  async reports(from?:string,to?:string) {
    ...
    return {period:{from:from??null,to:to??null},summary:{pneus:pneus.length,investment:pneus.reduce((a,p)=>a+number(p.valorCompra)+p.recapagens.reduce((x:any,r:any)=>x+number(r.valor),0)+p.consertos.reduce((x:any,c:any)=>x+number(c.valor),0),0),averageLifeKm:pneus.length?pneus.reduce((a,p)=>a+number(p.kmAtual),0)/pneus.length:0},history:history.sort((a,b)=>new Date(b.data).getTime()-new Date(a.data).getTime()),costsByVehicle:[...byVehicle.values()].sort((a,b)=>b.cost-a.cost),rankingBrands:[...byBrand.values()].sort((a,b)=>b.km-a.km),rankingRecappers:[...byRecapper.values()].sort((a,b)=>b.count-a.count),wear,nearReplacement:wear.filter(x=>x.sulcoAtual!=null&&x.sulcoAtual<=3)};
  }
};

...
export const motoristaBody = z.object({
  ...
  dataNascimento: dateOnly.optional().nullable().or(z.literal("")),
  ...
  salarioBase: money,
  dataAdmissao: dateOnly.optional().nullable().or(z.literal("")),
  cnhNumero: z.string().trim().max(40).optional().default(""),
...
export const veiculoBody = z.object({
  id: id.optional(),
  placa: text(20),
  modelo: optionalText(255),
  marca: z.string().trim().max(120).optional().default(""),
  renavam: z.string().trim().max(30).optional().default(""),
  chassi: z.string().trim().max(40).optional().default(""),
  anoFabricacao: z.coerce.number().int().min(1900).max(2200).optional().nullable(),
  anoModelo: z.coerce.number().int().min(1900).max(2200).optional().nullable(),
  ...
  quantidadeEstepes: z.coerce.number().int().min(0).max(3).default(1),
...
export const viagemBody = z.object({
  ...
  valorPedagio: money, valorDiaria: money, valorAbastecimento: money, abastecimentoId: id.optional().nullable(), abastecimentoIds: z.array(id).max(50).optional().default([]), valorComissao: money.optional().default(0), valorChapa: money, valorMulta: money.optional().default(0), custoExtraTag: z.string().trim().max(120).optional().default(""), valorCustoExtra: money.optional().default(0), observacoes: z.string().trim().max(5000).optional().default(""), createdAt: z.string().optional(),
});
...
export const pneuBody = z.object({
  id: id.optional(),
  numeroFogo: optionalText(100),
  marca: optionalText(120),
  modelo: optionalText(120),
  medida: optionalText(80),
  aro: optionalText(40),
  dot: optionalText(20),
  numeroSerie: optionalText(120),
  tipo: z.enum(["DIRECIONAL", "TRACAO", "LIVRE"]).optional().default("LIVRE"),
  valorCompra: money.optional().default(0),
  fornecedor: optionalText(180),
  dataCompra: dateOnly.optional().or(z.literal("")),
  maxRecapagens: z.coerce.number().int().min(0).max(20).optional().default(0),
  recapagensRealizadas: z.coerce.number().int().min(0).max(20).optional().default(0),
  status: z.enum(["ESTOQUE", "INSTALADO", "MANUTENCAO", "RECAPAGEM", "DESCARTADO"]).optional().default("ESTOQUE"),
  condicao: z.enum(["NOVO", "USADO", "RECAPADO", "AGUARDANDO_RECAPAGEM"]).optional().default("NOVO"),
  sulcoInicial: z.coerce.number().finite().min(0).max(100).optional().nullable(),
  sulcoAtual: z.coerce.number().finite().min(0).max(100).optional().nullable(),
  kmAtual: z.coerce.number().finite().min(0).optional().default(0),
  proximoRodizioKm: z.coerce.number().finite().min(0).optional().nullable(),
  observacoes: z.string().max(5000).optional().or(z.literal("")),
  fotos: z.array(z.string().max(20_000_000)).max(10).optional(),
  createdAt: z.string().optional(),
});

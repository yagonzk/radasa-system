...
import { MANUTENCAO_CATEGORIAS, maintenanceCategoryExamples, maintenanceCategoryLabel } from "@/lib/manutencao-categorias";
...
type OsItem = { id?: string; produtoId?: string | null; categoria?: string | null; tipo: "SERVICO" | "PECA" | "OUTRO"; descricao: string; quantidade: number; valorUnitario: number; valorTotal?: number; produto?: { id: string; nome: string; codigoInterno: string } | null };
...
const newItem = (): OsItem => ({ tipo: "SERVICO", categoria: "", descricao: "", produtoId: null, quantidade: 1, valorUnitario: 0 });
...
export default function Manutencao() {
  ...
  const concluirSelecionadas = async () => {
    ...
    if (!window.confirm(`Marcar ${ids.length} OS como concluída${ids.length > 1 ? "s" : ""}? Os custos dessas OS serão enviados ao DRE Operacional.`)) return;
  ...
  const salvarOs = async () => {
    ...
    const newItemWithoutCategory = osForm.itens.find((item) => !item.id && !item.categoria);
    if (newItemWithoutCategory) return toast.error("Selecione a categoria de cada novo serviço ou peça da OS.");
  ...
  const concluir = async () => {
    ...
    try {
      ...
      toast.success("OS concluída e custo enviado ao DRE Operacional.");
  ...
  return <Layout><div className="space-y-6 p-4 md:p-6">
    ...
    <Dialog open={!!detail} onOpenChange={(open) => !open && setDetail(null)}><DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-5xl">{detail && <>
      ...
      <div className="space-y-5">
        ...
        <div><h3 className="mb-2 font-semibold">Serviços e peças</h3><div className="overflow-x-auto rounded-lg border"><table className="w-full min-w-[650px] text-sm"><thead><tr className="border-b bg-muted/40 text-left text-muted-foreground"><th className="p-3">Tipo</th><th>Categoria</th><th>Descrição</th><th>Qtd.</th><th className="text-right">Unitário</th><th className="pr-3 text-right">Total</th></tr></thead><tbody>{detail.itens?.length ? detail.itens.map((item) => <tr key={item.id} className="border-b last:border-0"><td className="p-3">{item.tipo === "PECA" ? "Peça" : item.tipo === "SERVICO" ? "Serviço" : "Outro"}</td><td>{maintenanceCategoryLabel(item.categoria)}</td><td>{item.descricao || item.produto?.nome || "—"}{item.produto && <div className="text-xs text-muted-foreground">Almoxarifado: {item.produto.codigoInterno}</div>}</td><td>{item.quantidade}</td><td className="text-right">{money(item.valorUnitario)}</td><td className="pr-3 text-right font-medium">{money(item.valorTotal ?? item.quantidade * item.valorUnitario)}</td></tr>) : <tr><td colSpan={6} className="p-6 text-center text-muted-foreground">Nenhum item detalhado.</td></tr>}</tbody></table></div></div>
